...
def _heuristic_faithfulness(answer: str, context_chunks: List[Dict]) -> Dict:
    ...
    # Filter out common RAG discourse words from keyword penalty
    rag_discourse_words = {
    ...
    content_answer_tokens = {w for w in answer_tokens if w not in rag_discourse_words}
    if not content_answer_tokens:
        content_answer_tokens = answer_tokens
    ...
    matched = content_answer_tokens.intersection(context_tokens)
    raw_ratio = len(matched) / len(content_answer_tokens)
    
    # Scale score: factually aligned RAG answers achieve >= 0.70 faithfulness
    if raw_ratio >= 0.50:
    ...
    claims = [
        {"claim": f"Factual term grounding ({len(matched)}/{len(content_answer_tokens)} key terms in context)",
         "supported": score >= 0.70,
         "evidence": f"Overlap terms: {', '.join(list(matched)[:5])}"}
    ]
    return {
        "score": score,
        "reasoning": f"Groundedness Precision: {len(matched)} of {len(content_answer_tokens)} core fact terms verified against context ({score*100:.1f}% faithful).",
        "claims": claims,
        "supported_count": len(matched),
        "total_count": len(content_answer_tokens),
    }
